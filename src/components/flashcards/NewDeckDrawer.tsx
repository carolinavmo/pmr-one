...
import { createDeckAction } from "@/lib/actions/flashcards";
import { CARD_COLOR_SWATCH } from "@/lib/card-colors";
import type { CardColor } from "@/lib/editorial-blocks";
import { ColorSwatchPicker } from "@/components/ui/ColorSwatchPicker";
import { Button } from "@/components/ui/Button";

// Structurally modeled on TaskFormDrawer.tsx, not on AtlasEditor's
// rich-text pattern — a deck name + color is short structured input,
// not long-form notes, so a plain form is the right shape here.
export function NewDeckDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Reset the form whenever the drawer (re)opens, discarding any
  // unsaved input from a previous open — adjusted during render
  // (React's documented pattern for resetting state on a prop change)
  // rather than in a useEffect, same as TaskFormDrawer.tsx.
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      await createDeckAction(name, color);
      onClose();
    });
  }
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between">
          <h2 className="font-heading text-base font-semibold text-primary">{t("newDeckTitle")}</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label={t("cancel")}
        ...
        <label className="flex flex-col gap-1.5">
          <span className="font-ui text-xs text-secondary">{t("deckNameLabel")}</span>
          <input
            type="text"
            value={name}
