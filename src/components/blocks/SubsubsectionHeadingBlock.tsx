...
// but no tinted pill background — just colored, smaller, bold text on
// its own line. Deliberately no id/slugify and no
// notifySectionIndexChanged, same reasoning as the subsection tier:
// not meant to be deep-linkable or counted, only visually styled like
// a heading within the surrounding section's own content.
export function SubsubsectionHeadingBlockView({ block }: { block: SubsubsectionHeadingBlock }) {
  ...
  return (
    <EditableText
      as="h4"
      multiline={false}
      className={`font-heading text-base font-semibold text-[#128A99] ${TEXT_ALIGN_CLASS[textAlign]}`}
      value={block.text}
