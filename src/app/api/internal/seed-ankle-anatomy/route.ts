...
// One-time production seed for the Ankle Anatomy disease page,
// imported from Ankle-Anatomy-PMR-Chapter.docx — same pattern as the
// earlier Elbow/Knee/Hip Anatomy imports. Idempotent: if the disease
// already exists, this is a no-op rather than a duplicate insert.
export async function GET() {
  const { rows: existing } = await pool.query(`SELECT id FROM disease WHERE slug = 'ankle-anatomy'`);
  if (existing[0]) {
  ...
  const { rows: diseaseRows } = await pool.query(
    `INSERT INTO disease (canonical_name, slug, status, topic_id, source_locale, evidence_based, position)
     VALUES ('Ankle Anatomy', 'ankle-anatomy', 'published', $1, 'en', true, $2)
     RETURNING id`,
    [topicId, position]
  );
