...
// One-time production seed for the Ankle and Foot Anatomy disease
// page, imported from Ankle-and-Foot-Anatomy-PMR-Chapter.docx — a
// separate, broader page from the existing "Ankle Anatomy" one (adds
// arches/windlass, forefoot, intrinsic muscles, the diabetic foot,
// etc.). Same pattern as the earlier Elbow/Knee/Hip/Ankle imports.
// Idempotent: if the disease already exists, this is a no-op rather
// than a duplicate insert.
export async function GET() {
  const { rows: existing } = await pool.query(
    `SELECT id FROM disease WHERE slug = 'ankle-and-foot-anatomy'`
  );
  if (existing[0]) {
  ...
  const { rows: diseaseRows } = await pool.query(
    `INSERT INTO disease (canonical_name, slug, status, topic_id, source_locale, evidence_based, position)
     VALUES ('Ankle and Foot Anatomy', 'ankle-and-foot-anatomy', 'published', $1, 'en', true, $2)
     RETURNING id`,
    [topicId, position]
  );
